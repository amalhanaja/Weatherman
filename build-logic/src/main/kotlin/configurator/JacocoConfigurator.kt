...
internal fun JacocoReport.configureJacoco(project: Project, variantName: String?) {
    ...
    val androidJavaClasses = projects
        .map { proj -> project.fileTree("${proj.buildDir}/intermediates/javac/$variantName/classes") }
        .onEach { it.exclude(coverageExclusions) }
    val androidKotlin = projects
        .map { proj -> project.fileTree("${proj.buildDir}/tmp/kotlin-classes/$variantName") }
        .onEach { it.exclude(coverageExclusions) }
    val nativeKotlin = projects.map { proj ->
        proj.fileTree("${proj.buildDir}/classes")
    }.onEach { it.exclude(coverageExclusions) }

    classDirectories.setFrom(project.files(androidJavaClasses, androidKotlin, nativeKotlin))
